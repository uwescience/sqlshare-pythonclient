...
"""
a class that wraps up the html code and html page
by passing in a dict return from the server(by calling execute_sql)
"""
class HTMLWrapper:
  _html_code_ = ""
  _html_header_ = ""
  ...
  def __init__(self,  raw_dict, header=None, data=None):
    if (not raw_dict == None) and header == None and data == None:
      headers = raw_dict["header"]
      self._html_header_ = "<tr>"
      #handle the header
      for header in headers:
        self._html_header_ += "<td>" + `header` + "</td>"
      self._html_header_ += "</tr>"
      ...
      for line in data:
        self._html_data_ += "<tr>"
        for item in line:
           self._html_data_ +="<td>" + `item` + "</td>"
        self._html_data_ += "</tr>"
      self._html_code_ = "<table>" + self._html_header_ + self._html_data_ + "</table>"
      self._html_page_ = HTML(self._html_code_)
    elif (raw_dict == None) and not header == None and not data == None:
      self._html_header_ = header
      self._html_data_ = data
      self._html_code_ = "<table>" + self._html_header_ + self._html_data_ + "</table>"
      self._html_page_ = HTML(self._html_code_)
  ...
  def get_html_code(self):
    return self._html_code_

  def get_html_page(self):
    return self._html_page_

  def get_html_data(self):
    return self._html_data_

  def get_html_header(self):
    return self._html_header_

  def merge_another(self, other):
    new_data = self._html_data_ + other.get_html_data()
    return HTMLWrapper(None, self._html_header_, new_data)
